...
func (t *AppClientZapsDBDao) InitializeDao(client utils.Map) {
	...
	collection, err := connection.GetCollection(platform_common.DbPlatformAppClients)
	if err != nil {
		log.Println("SysClient Table not found, createTable in ZapsDB")
		collection, err = connection.CreateCollection(platform_common.DbPlatformAppClients, platform_common.FLD_CLIENT_ID, "Application User Profile")
		if err != nil {
...
func (t *AppClientZapsDBDao) List(filter string, sort string, skip int64, limit int64) (utils.Map, error) {
	...
	connection, _ := zapsdb_utils.GetConnection(t.client)
	dataresponse, err := connection.GetMany(platform_common.DbPlatformAppClients, filter, sort, skip, limit)
	if err != nil {
...
func (t *AppClientZapsDBDao) Get(clientid string) (utils.Map, error) {
	...
	connection, _ := zapsdb_utils.GetConnection(t.client)
	singleResult, err := connection.GetOne(platform_common.DbPlatformAppClients, clientid, "")
	if err != nil {
...
func (t *AppClientZapsDBDao) Find(filter string) (utils.Map, error) {
	...
	connection, _ := zapsdb_utils.GetConnection(t.client)
	singleResult, err := connection.FindOne(platform_common.DbPlatformAppClients, filter, "")
	if err != nil {
		log.Println("Find:: Record not found ", err)
		return result, err
	}
...
func (t *AppClientZapsDBDao) Create(indata utils.Map) (string, error) {
	...
	insertResult, err := connection.Insert(platform_common.DbPlatformAppClients, indata, txnid)
	if err != nil {
...
func (t *AppClientZapsDBDao) Update(clientid string, indata utils.Map) (utils.Map, error) {
	...
	connection, txnid := zapsdb_utils.GetConnection(t.client)
	updateResult, err := connection.UpdateOne(platform_common.DbPlatformAppClients, clientid, indata, txnid)
	if err != nil {
...
func (t *AppClientZapsDBDao) Delete(clientid string) (int64, error) {
	...
	connection, txnid := zapsdb_utils.GetConnection(t.client)
	res, err := connection.DeleteOne(platform_common.DbPlatformAppClients, clientid, txnid)
	if err != nil {
