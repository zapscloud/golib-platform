...
func NewAppClientService(props utils.Map) (AppClientService, error) {
	...
	p.daoAppClient = platform_repository.NewAppClientDao(p.GetClient())
	p.child = &p
	return &p, nil
}
...
func (p *appClientBaseService) List(filter string, sort string, skip int64, limit int64) (utils.Map, error) {
	...
	log.Println("AppClientService::FindAll - Begin")

	dataresponse, err := p.daoAppClient.List(filter, sort, skip, limit)
	if err != nil {
		return nil, err
	}
	log.Println("AppClientService::FindAll - End ")
	return dataresponse, nil
}

// GetDetails - Find By Code
func (p *appClientBaseService) GetDetails(clientid string) (utils.Map, error) {
	log.Printf("AppClientService::GetDetails::  Begin %v", clientid)

	data, err := p.daoAppClient.GetDetails(clientid)

	log.Println("AppClientService::GetDetails:: End ", data, err)
	return data, err
}

func (p *appClientBaseService) Find(filter string) (utils.Map, error) {
	fmt.Println("AppClientService::GetDetails::  Begin ", filter)
